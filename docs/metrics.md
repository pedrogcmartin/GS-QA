...
The objective quality metrics selection was based on several criteria, notably: i) metrics that are typically applied in the GS literature, namely PSNR-Y, PSNR-YUV, SSIM [1], MS-SSIM [2], LPIPS [3], and FovVideoVDP [4], since it is important to know whether they are in fact suitable for assessing the quality of GS content; ii) metrics that have shown good performance in several image and video applications, such as PSNR-HVS [5], IW-SSIM [6], VIF and VIFp [7], FSIM [8], VSI [9], MAD [10], GMSD [11], and NLPD [12]; iii) learning-based metrics (besides LPIPS), namely ST-LPIPS [13], DISTS [14], and VMAF [15]; iv) MSE, since every selected GS method includes it in the loss function. All metrics are full-reference.
